...
class MetricsProvider(Protocol):
    ...
    def has_family(self, family: str) -> bool:
        ...
        **它只回答"族在不在"**，不回答"能不能画出某个字符"——后者是
        `has_glyph`。两者必须分开：`Microsoft YaHei` 装是装了，
        但它画不出日文假名，回退链需要知道这个区别。
        """

    def has_glyph(self, family: str, char: str) -> bool:
        """这个族**能不能画出这个字符**（逐字符覆盖探测）。

        为什么需要它：只按"族存不存在"做回退，会出现"族在、但画不出"——
        表现是明明装了中文字体，日文假名却全是豆腐块。有了它，
        `text/fallback.py` 的逐脚本选族才能从"族存在"升级为"族覆盖该脚本"。

        只探测**基字符**（簇的第一个码点）：组合符跟随基字符，
