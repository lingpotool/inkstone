...
>
> **R7「交互闭环与 Phase 1 收尾」（docs/21）已整包完成（R7.1–R7.5）**。
> R7.1 命中测试与指针路由；R7.2 手势竞技场；R7.3 DPI 缩放接线；
> R7.4 样板 App「墨记」（`examples/notes.py`，`ScrollView` widget 补齐，
> headless 黄金图 + `--sdl2`）；R7.5 性能基准（`benchmarks/run.py`，三场景
> p50/p95）。R7.4 逼出并修掉文本栈一个真 bug（多字重下光栅选错字体面，
> ADR-0016）。
>
> **R7.5 决策规则被数据触发 → GL 后端子包已立项**（`docs/22`）：三场景 p95
> 超预算 40–300 倍（滚动 859ms / 全屏 3125ms / 文本 576ms，预算 10ms），
> 热点是软件光栅的纯 Python 逐像素 SDF。GL 包只实现现有显示列表 IR、
> 沿用 R3.1 帧生命周期、不改 core、黄金图仍以软件光栅为事实源。
> 无头单测 **888 → 944** 全绿，覆盖率 89.8%。
>
> **下一个该动的是 GL 后端子包（docs/22）**；Phase 1 剩余 DoD 另有两项：
> 文本编辑模型（可输入/可删除/光标，Phase 2 首项）、macOS/Linux 真机字体验证
> 与三平台 SDL2 交互验证。
>
...
> R7「交互闭环与 Phase 1 收尾」（docs/21）**已整包完成**（944 测试全绿）。
> 当前该动的是 **GL 后端子包（docs/22）**——R7.5 的基准数据触发了预先写死的
> 规则；Phase 1 剩余 DoD 是文本编辑模型与 macOS/Linux 真机验证。
