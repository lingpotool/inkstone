...
>
> **R7「交互闭环与 Phase 1 收尾」（docs/21）进行中：R7.1–R7.4 已完成**。
> R7.1 命中测试与指针路由；R7.2 手势竞技场；R7.3 DPI 缩放接线；
> R7.4 样板 App「墨记」（`examples/notes.py`：侧栏 + 滚动列表 + 表单 +
> 明暗主题切换，`ScrollView` widget 补齐，headless 黄金图 `notes_light` /
> `notes_dark` 进 CI、`--sdl2` 真窗口交互）。R7.4 还逼出并修掉文本栈一个真
> bug：多字重字体下光栅按 family 重选面、用甲的 glyph id 查乙的轮廓，
> 粗体整行画成别的字（ADR-0016）。
> 无头单测 **888 → 944** 全绿，覆盖率 89.8%。
>
> **下一个该动的是 R7.5「性能基准 → GL 后端决策」**（docs/21）：在
> `benchmarks/` 跑长列表滚动 / 全屏重绘 / 文本密集页三个场景，出 p50/p95；
> **先写死决策规则再量**：p95 > 10ms 就另立 GL 后端子包，否则记入
> docs/20 §R6.4 后补清单，GL 让位于 Phase 2 铺组件。
>
...
> R7「交互闭环与 Phase 1 收尾」（docs/21）进行中：**R7.1–R7.4 已完成**
> （944 测试全绿），当前该动的是 **R7.5 性能基准 → GL 后端决策**。
> docs/21 是下面这几节的正式施工版，以它为准。
