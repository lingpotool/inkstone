...
988 个无头单测全绿，**黄金图像素级比对**也跑通（12 张基线）。
...
GL 子包 R8.1（驱动接缝 + 后端逻辑层）与 R8.2（Windows WGL 真机驱动）已完成，
测试 → 988 全绿（覆盖率 89.99%）；R8.3 进行中：热路径缓存（段落/脚本判定）
+ 矩形合批 + 字形图集，GL p95 全屏 28ms / 文本 45ms / 滚动 153ms，
仍未达 10ms（瓶颈：合批被文本交替切断、滚动无 repaint boundary）。
