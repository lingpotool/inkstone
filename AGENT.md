...
1152 个无头单测全绿，**黄金图像素级比对**也跑通（12 张基线）。
...
测试 → 1152 全绿（覆盖率 89.93%）。**R7.5 验收达成：GL p95 全屏 4.4ms /
...
| ADR-0026 | **滚动条让出槽位（gutter），绝不压在内容上；`overflow: auto` 语义** | 第一版做成了"常驻半透明覆盖条"，用户一眼看穿：**它盖在列表条目上**。这是最差的组合——既有 overlay 的遮挡，又没有 overlay 的自动淡出。专业实现的第一原则是**滚动条绝不遮挡内容**，两条主流路线：①Chromium/VS Code 的 overlay 条——只在滚动/悬停时出现、约 1s 后淡出（需要动画基础设施，`motion/` 目前是空的）；②Win32 经典 / Qt / CSS `overflow: auto`——在视口里让出一条槽位，内容按"视口宽 − 滚动条宽"排版。选 ②：**只有真溢出时才让位**（`RenderScroll.perform_layout` 先按满宽量一次，溢出则扣掉 `thickness+margin` 再量一次）。二次布局是**有界的一次**（更窄 ⇒ 内容只会更高，不会反转），不是迭代收敛；且滚动本身不触发重排（R8.4 只改偏移），所以代价只落在真正发生布局的帧上。实测：150 项列表整树重排 p50 1.86→3.69ms，而基准的滚动场景 p50 0.27→0.47ms，仍远低于 8ms 预算。自动淡出留给 `motion/` 落地后作为可选外观（令牌已分离几何与颜色，接口不用改） |
