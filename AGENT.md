...
991 个无头单测全绿，**黄金图像素级比对**也跑通（12 张基线）。
...
R8.3（热路径缓存 + 矩形合批 + 字形图集 + 帧去重）已完成，
测试 → 991 全绿（覆盖率 89.99%）。GL p95：全屏 5.7ms ✅ / 文本 8.8ms ✅ /
滚动 108ms ✗——滚动每帧平移、帧去重命中不了，需 repaint boundary + 变换指令
（Flutter 层模型，见 docs/22 §8）。
