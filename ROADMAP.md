...
8. **测试基建**：布局单测 + 黄金图测试（三平台基线）+ CI
   - **进度（基本完成）**：662 个无头单测全绿；7 张黄金图（登录表单明暗、
     Button / Card / Input 单独、文本块、文本换行）已建立并**按解码后的像素**
     逐像素比对。devtools 强制 `force_repaint` 让黄金图每帧都是完整画面，
     不污染生产帧的"跳干净子树"优化。
   ...
   - 待补：gl_backend / skia_backend、真字形渲染。
