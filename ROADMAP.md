...
2. **渲染管线**：显示列表 + 自研 GL 光栅后端（先不引入 Skia，减少变量；Skia 属 Phase 3）
   ...
   - **进度（部分完成）**：显示列表（含 `TextRunOp` / path 指令）、仿射录制器、
     ...
     **自研 GL 光栅后端**（R7.5 基准触发，立项 `docs/22`）：R8.1 驱动接缝 +
     ...
     帧去重、R8.4 状态指令 + 滚动 repaint boundary + GL 层缓存、R8.6 真窗口 GL 上屏已完成。
     **R7.5 验收达成**：三场景 p95 ≤10ms（4.4 / 8.6 / 1.0ms）；R12.2 预计算
     已定位字形后进一步降到 ~2.2 / ~1.6 / ~0.6ms。尚缺：Linux GLX/EGL 与
     macOS CGL 驱动、路径三角化、通用脏矩形损伤跟踪、
...
8. **测试基建**：布局单测 + 黄金图测试（三平台基线）+ CI
   - **进度（基本完成）**：**1152 个无头单测全绿**（覆盖率 89.93%）；
   ...
   - gl_backend **已完成**（R8.1–R8.6，见 docs/22 与 ADR-0017–0021）：驱动接缝 +
     GLRasterBackend 逻辑层 + Windows WGL 真机驱动 + 热路径/合批/图集/帧去重 +
     状态指令与滚动层缓存 + SDL 真窗口上屏。待补：Linux GLX/EGL、macOS CGL、
     路径三角化；skia_backend（Phase 3）。
...
- [x] 中文输入：可输入、可删除、光标位置正确
      —— R7.1 接通"点击聚焦 + IME 通道"；R9 落地**焦点系统**（唯一焦点 /
      focus-visible / Tab 遍历 / 点空白失焦）与**编辑模型**（text + selection
      + composition，按字素簇移动删除、光标/选区渲染、组合态下划线、剪贴板、
      候选框跟随光标）。更复杂的组合态高亮属 Phase 2 打磨
- [x] 100%/125%/150% 缩放下无模糊、无错位
      —— R11 修掉进程 DPI 不感知（Windows 会把整窗位图拉伸）；本机 125%
      实测窗口客户区 950×650 物理 = 760×520 逻辑 × 1.25，渲染与显示 1:1
- [x] 布局引擎 100% 无窗口可测，单测覆盖 ≥ 85%（实测 89.93%）
- [x] 样板 App（一个真实小工具）**在 Windows 上可用**
      —— R7.4 交付 `examples/notes.py`；R10 接窗口身份/图标/主题跟随与可缩放
      视口；R12.1 补上滚轮、R12.3 逐项真机验收（清单见 HANDOFF）。
      **macOS / Linux 待适配 ⬜**（ADR-0025：跨平台适配独立成工作流）
- [x] 帧时间 p95 < 8ms（200 节点界面）
      —— 软件光栅曾超预算两个数量级（R7.5），GL 后端落地（docs/22）后三场景
      ≤10ms；R12.2 预计算已定位字形，本机 p95 进一步降到
      **滚动 ~0.6 / 全屏 ~2.2 / 文本 ~1.6ms**（场景定义未动）
