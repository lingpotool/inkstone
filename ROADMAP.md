...
2. **渲染管线**：显示列表 + 自研 GL 光栅后端（先不引入 Skia，减少变量；Skia 属 Phase 3）
   ...
   - **进度（部分完成）**：显示列表（含 `TextRunOp` / path 指令）、仿射录制器、
     ...
     **R7.5 验收达成**：本机 p95 全屏 4.4ms / 文本 8.6ms / 滚动 1.0ms，
     三场景 ≤10ms。尚缺：Linux GLX/EGL 与 macOS CGL 驱动、路径三角化、通用脏矩形损伤跟踪、
...
8. **测试基建**：布局单测 + 黄金图测试（三平台基线）+ CI
   - **进度（基本完成）**：**1089 个无头单测全绿**（覆盖率 90.11%）；
...
- [x] 布局引擎 100% 无窗口可测，单测覆盖 ≥ 85%（实测 90.11%）
- [ ] 样板 App（一个真实小工具）在三平台可用
      —— R7.4 已交付 `examples/notes.py`（headless 出图 + SDL2 路径）；
      R10 已接窗口身份/图标/主题跟随与可缩放视口；SDL2 真机交互尚待在
      Windows/macOS/Linux 上各验一遍
- [ ] 帧时间 p95 < 8ms（200 节点界面）
      —— 软件光栅实测 p95 超预算两个数量级（R7.5），GL 后端已落地（docs/22）：
      本机 RTX 3060 实测 p95 滚动 1.0ms / 全屏 4.4ms / 文本 8.6ms。
      全屏与滚动已进 8ms，**文本场景 8.6ms 仍差一点**——待后续热路径优化后勾选
