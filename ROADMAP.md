...
2. **渲染管线**：显示列表 + 自研 GL 光栅后端（先不引入 Skia，减少变量；Skia 属 Phase 3）
   ...
   - **进度（部分完成）**：显示列表（含 `TextRunOp` / path 指令）、仿射录制器、
     ...
     **自研 GL 光栅后端**（R7.5 基准触发，立项 `docs/22`）：R8.1 驱动接缝 +
     后端逻辑层、R8.2 Windows WGL 真机驱动（`backend/gl_wgl.py`，离屏 FBO）
     已完成——实测比软件快 5–90 倍（全屏 p95 3125→34ms），但离 p95 ≤ 10ms
     还差，R8.3 做字形图集与合批、路径三角化。尚缺：Linux GLX/EGL 与
     macOS CGL 驱动、真窗口 present、阴影、渐变、彩色 emoji。
     **注意**：软件光栅是黄金图的确定性事实源，不是帧率后端
     （三场景 p95 超预算 40–300 倍，见 docs/22）。
