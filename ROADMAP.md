...
2. **渲染管线**：显示列表 + 自研 GL 光栅后端（先不引入 Skia，减少变量；Skia 属 Phase 3）
   ...
   - **进度（部分完成）**：显示列表（含 `TextRunOp` / path 指令）、仿射录制器、
     ...
     **自研 GL 光栅后端**（R7.5 基准触发，立项 `docs/22`）：R8.1 已完成
     驱动接缝（`GLDriver`）与后端逻辑层（`GLRasterBackend`，假驱动 30 例）；
     R8.2 的 ctypes 真机驱动需要能验证 GL 的环境（本机/CI 无 GPU）。
     尚缺：真机 GL 驱动、图集与批处理、路径与软件光栅同步落地、阴影、渐变、
     彩色 emoji。**注意**：软件光栅是黄金图的确定性事实源，不是帧率后端
